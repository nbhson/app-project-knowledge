...
async def test_extraction_pipeline_rule_based(tmp_path):
    ...
    items = [
        RawItem(item_id="test.py", source_type="GIT", title="test.py", content="class Foo:\n    def bar(self): pass", content_type="python"),
        RawItem(item_id="doc.md", source_type="DOCUMENT", title="ADR-001", content="# ADR-001\nContext\nDecision\nConsequences", content_type="markdown"),
    ]

...
async def test_context_assembly(tmp_path):
    ...
    ko = KnowledgeObject(object_type=ObjectType.ENTITY, entity_type=EntityType.CLASS, title="PaymentService", content="Handles payments", source_references=[sr], confidence=0.9)
    await store.save(ko)
    assembler = ContextAssembler(store)
    package = await assembler.assemble("How does PaymentService work?", [(ko, 0.9)], intent="CODE_UNDERSTANDING", search_stats=SearchStats(vector_results=1))
...
def test_compression():
    ...
    chunks = [
        KnowledgeChunk(id=str(i), type="CLASS", title=f"T{i}", content="x " * 2000, confidence=0.9, lifecycle_state=LifecycleState.ACTIVE, relevance_score=1.0 - i * 0.1, rank=i, sources=[])
        for i in range(5)
    ]
    pkg = ContextPackage(query="q", knowledge=chunks, relationships=[], confidence=0.9, sources=[], lifecycle_states=["ACTIVE"], search_stats=SearchStats())
...
def test_validator():
    ...
    chunk = KnowledgeChunk(id="1", type="CLASS", title="T", content="hello", confidence=0.9, lifecycle_state="ACTIVE", relevance_score=1.0, rank=1, sources=[])
    pkg = ContextPackage(query="q", knowledge=[chunk], relationships=[], confidence=0.9, sources=[], lifecycle_states=["ACTIVE"])
    v = ContextValidator()
